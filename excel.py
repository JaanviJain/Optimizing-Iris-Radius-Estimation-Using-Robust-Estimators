...
from skimage.filters import sobel, scharr, threshold_otsu, unsharp_mask
from skimage.segmentation import morphological_geodesic_active_contour, clear_border
from scipy import ndimage
from scipy.signal import savgol_filter

# ========== SEGMENTATION FUNCTIONS (from your code) ==========
...
def segment_iris_gac_only(image_path):
    ...
    return bgr, gray, pupil, limbus, cx, cy

# ========== NEW: MEASUREMENT AND EXCEL RECORDING ==========
def get_radius_at_angle(limbus_mask, pupil_center, angle_degrees):
    """
    Calculate the distance from pupil center to limbus boundary at a specific angle.
    ...
    # Create a ray from pupil center in the angle direction
    h, w = limbus_mask.shape
    max_radius = min(h, w)  # Maximum possible radius
    ...
    for r in range(1, int(max_radius)):
        ...
        if x < 0 or x >= w or y < 0 or y >= h:
            return r - 1  # Return last valid radius
        
        # Check if we hit the limbus boundary
        if not limbus_mask[y, x]:
            return r
    
    return max_radius  # Return max radius if no boundary found

def process_all_images(base_directory, output_excel_path):
    """
    Process all images in the directory structure and save results to Excel.
    Directory structure: base_directory / person_folder / [left, right] / image_files
    """
    
    # List to store all measurement data
    all_data = []
    
    # Define the angles we want to measure (your specified angles + the new ones)
    angles = [30, 0, -30, 150, 180, 210, 90, 270]
    ...
    for person_folder in os.listdir(base_directory):
        ...
        if os.path.isdir(person_path):
            for eye_folder in ['left', 'right']:
                eye_path = os.path.join(person_path, eye_folder)
                if os.path.exists(eye_path):
                    image_files = [f for f in os.listdir(eye_path) 
                                 if f.lower().endswith(('.bmp', '.jpg', '.jpeg', '.png', '.tiff'))]
    ...
    for person_folder in os.listdir(base_directory):
        ...
        for eye_folder in ['left', 'right']:
            ...
            # Get all image files in the eye folder
            image_files = [f for f in os.listdir(eye_path) 
                         if f.lower().endswith(('.bmp', '.jpg', '.jpeg', '.png', '.tiff'))]
            ...
            for image_file in image_files:
                ...
                try:
                    # Segment the iris
                    bgr, gray, pupil, limbus, pupil_x, pupil_y = segment_iris_gac_only(image_path)
                    
                    # Get limbus center (centroid of limbus mask)
                    limbus_props = measure.regionprops(limbus.astype(int))
                    if limbus_props:
                        limbus_y, limbus_x = limbus_props[0].centroid
                    else:
                        limbus_x, limbus_y = pupil_x, pupil_y  # Fallback to pupil center
                    
                    # Measure radii at all specified angles
                    radius_data = {}
                    pupil_center = (pupil_x, pupil_y)
                    
                    for angle in angles:
                        radius = get_radius_at_angle(limbus, pupil_center, angle)
                        radius_data[f'radius_{angle}'] = radius
                    ...
                    # Add radius measurements
                    row_data.update(radius_data)
                    
                    all_data.append(row_data)
                    processed_images += 1
                    
                    print(f"Processed {processed_images}/{total_images}: {person_folder}/{eye_folder}/{image_file}")
    ...
    if all_data:
        ...
        columns = ['person_id', 'eye', 'image_id', 'pupil_x', 'pupil_y', 'limbus_x', 'limbus_y']
        radius_columns = [f'radius_{angle}' for angle in angles]
        columns.extend(radius_columns)
        ...
        print(f"Processed {len(all_data)} images out of {total_images} total")
...
if __name__ == "__main__":
    # === CONFIGURATION ===
    BASE_DIRECTORY = r"D:\SEM-5\ELECTIVE'S\DEPARTMENT-ELECTIVE\BIOMETRICS\Project\Final_Dataset"  # Path to your main dataset folder
    OUTPUT_EXCEL_PATH = "iris_measurements.xlsx"  # Output Excel file name
    
    # Process all images and create Excel file
    results_df = process_all_images(BASE_DIRECTORY, OUTPUT_EXCEL_PATH)
